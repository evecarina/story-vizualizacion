...
const visualizacionMapa = {
  ...
  loadData() {
    var xmlhttp = new XMLHttpRequest()
    xmlhttp.open('GET', 'data/proyecto-hepatitis-2.json', true)
    xmlhttp.onreadystatechange = () => {
      if (xmlhttp.readyState == 4) {
        if(xmlhttp.status == 200) {
  ...
  createListProductos() {
    ...
    _.map(_.groupBy(allData, 'Class'), (cantidad,class_resultados) => {
      const productoItem = {
        "class_resultados": class_resultados,
        "cantidad": cantidad.length
      }

      listContent += `<li><a class="dropdown-item" href="#" onclick="visualizacionMapa.cargarGrafico('${productoItem.class_resultados}')">${productoItem.class_resultados} (${productoItem.cantidad})</a></li>`
    })
...
// </div>
  ...
  cargarGrafico(class_resultados) {
    visualizacionMapa.cargarBarras(class_resultados)
    visualizacionMapa.cargarPie(provincia)
    visualizacionMapa.cargarHistogram(provincia)

    // const class_resultados = _.find(allData, ['Class', class_resultados])

    // map.setView([provinciaData, provinciaData], 9)

  },

  cargarBarras(nombre_class_resultados) {

    const data_class_resultados = _.filter(allData, ['Class', nombre_class_resultados])

    const cantidadProductos = _.map(_.groupBy(data_class_resultados, 'SEX'), sexo => {
      return sexo.length
    })

    // const totalProductos = _.map(_.groupBy(data_class_resultados, 'SEX'), provincia => {
    ...
    Highcharts.chart('totalProductos', {
      ...
      title: {
          text: 'Reporte General de personas Vivas y Muertas'
      },
      subtitle: {
          text: ''
      },
      xAxis: {
        categories: _.uniq(_.map(data_class_resultados, 'SEX')),
        crosshair: true
      },
      yAxis: {
          min: 0,
